...
export function Column({ column, projectId, users, onEditTask, isDoneColumn, isReviewColumn, userRole, isFlashing, pushId, highlightTaskId, currentUserId }: ColumnProps) {
    ...
    return (
        <div
            ref={setDroppableRef}
            className={`relative flex h-full w-full min-w-0 flex-col rounded-lg p-3 transition-all min-h-[150px] ${getBgClass()} ${isDropDisabled ? 'opacity-50' : ''}`}
            style={{ zIndex: 5 }}
        >
                ...
                <SortableContext items={column.tasks.map(t => t.id)} strategy={verticalListSortingStrategy}>
                    {column.tasks.map(task => (
                        <TaskCard
                            ...
                            projectId={projectId}
                            validAssigneeUserIds={validAssigneeUserIds}
                        />
                    ))}
                </SortableContext>
